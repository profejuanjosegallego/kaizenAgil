...
export const authService = {
  ...
  // Actualiza datos del propio perfil (por ahora, el nombre).
  async updateProfile(userId, { name }) {
    ...
    const user = await userRepository.update(userId, { name: clean });
  ...
  // `baseUrl` viene del dominio real de la petición (así el enlace usa el dominio
  // correcto en Vercel sin depender de NEXT_PUBLIC_SITE_URL).
  async requestReset(email, baseUrl) {
    ...
    if (user) {
      ...
      const site = String(baseUrl || SITE_URL).replace(/\/$/, "");
